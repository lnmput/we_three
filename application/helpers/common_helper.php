 ...
 * @parma Precision
 */
function getKeyRequestAdvancedLocationMessage(){
	 return array(
	 		 "latitude"=>getRequest('latitude'),
	 		 "longitude"=>getRequest('longitude'),
	 		 "precision"=>getRequest('precision')	 
	 		);
}
/*
 * 获得点击菜单消息的关键参数
 ...
 * @parma msgid
 */
function getKeyRequestAdvancedVoiceMessage(){
    	return array(
			"mediaid"=>getRequest('mediaid'),
			"format"=>getRequest('format'),
			"msgid"=>getRequest('msgid'),
            "recognition"=>getRequest('recognition')
			);
}
/*
 * 获得扫描带参数二维码的关键参数
 * @parma eventKey 事件KEY值，是一个32位无符号整数，即创建二维码时的二维码scene_id
 * @parma ticket 二维码的ticket，可用来换取二维码图片
 */
function getKeyRequestQrcodeMessage(){
	return array(
			"eventKey"=>getRequest('eventKey'),
 ...
 * 
 */
function getKeySubscribeEventMessage(){
	return array(
